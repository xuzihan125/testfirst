...
#
n_timestamp = 10
train_days = 11000  # number of days to train from
...
url = 'D:\\www\\main\\testfirst\\resource\\weather-info\\6.csv'
dataset = pd.read_csv(url)
if filter_on == 1:
    dataset['tmax'] = medfilt(dataset['tmax'], 3)
    dataset['tmax'] = gaussian_filter1d(dataset['tmax'], 1.2)
...
model.save('D:\\www\\main\\testfirst\\resource\\prediction\\LSTM\\LSTM-model.h5')

model = load_model('D:\\www\\main\\testfirst\\resource\\prediction\\LSTM\\LSTM-model.h5')
...
plt.subplot(3, 1, 1)
plt.plot(dataset['tmax'], color = 'black', linewidth=1, label = 'True value')
plt.ylabel("tmax")
plt.xlabel("Day")
plt.title("All data")
...
'''
truet = dataset[11000:11000 + date]
truet = truet.iloc[:, 1:2].values
# print(truet)
# print('-------------')
# print(y[n_timestamp:])
# print('-------------')
# print(dataset['tmax'][11000-n_timestamp:11000+date])
mse = mean_squared_error(truet, y[n_timestamp:])
print(mse)
'''
pd.DataFrame(y).to_csv('D:\\www\\main\\testfirst\\resource\\prediction\\LSTM\\predict_data-tempo.csv')
